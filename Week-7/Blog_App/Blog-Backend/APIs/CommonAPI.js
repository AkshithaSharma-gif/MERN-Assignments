...
commonApp.put("/password",verifyToken,async (req,res)=>{
    ...
    if(currentPassword===newPassword){
        res.status(404).json({message:"Current password and new password cannot be same"})
    }
    //get current password of user
    const userId=req.user?.id;
    //find the user
    let user=UserModel.findById(userId)
    //check current password of req and user is same
    const compare=await compare(user.password,currentPassword)
    if(!compare){
